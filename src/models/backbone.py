...
class DINOv2Backbone(nn.Module):
    """Frozen DINOv2 backbone returning CLS token and patch tokens."""
    ...
    def train(self, mode: bool = True) -> DINOv2Backbone:
        super().train(mode)
        self._dino.eval()  # backbone은 학습 중에도 eval 유지
        return self
...
class EVA02Backbone(nn.Module):
    """Frozen EVA-02 backbone (via timm) returning CLS token and patch tokens.

    timm lazy import: DINOv2 전용 환경에서 timm 미설치 시에도 모듈을 import할 수 있다.
    forward_features → [B, N+1, D] where index 0 = CLS, 1: = patch tokens.
    """

    def __init__(self, model_name: str = "eva02_base_patch14_448") -> None:
        super().__init__()
        if model_name not in BACKBONE_REGISTRY:
            raise ValueError(f"Unknown backbone: {model_name}. Choose from {list(BACKBONE_REGISTRY)}")
...
def build_backbone(model_name: str) -> DINOv2Backbone | EVA02Backbone:
    """Backbone 팩토리: eva02_* → EVA02Backbone, 나머지 → DINOv2Backbone."""
    if model_name in _EVA02_NAMES:
        return EVA02Backbone(model_name)
    return DINOv2Backbone(model_name)
