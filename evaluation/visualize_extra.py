#!/usr/bin/env python3
"""Presentation-grade extra figures for the hazard-detection model (no re-inference).

기존 evaluate_test.py / report_artifacts.py 가 만든 results/{exp}/{target}/ 산출물
(predictions.csv, summary.csv)을 그대로 재사용해, 팀원 시각화(steel-scrap의
visualize_results_improved.py)에서 *추가된* 발표용 그림만 내 포맷에 맞게 포팅한다.

출력은 experiment namespace를 유지하도록 results/{exp}/presentation/ 하위에 저장한다
(--out 미지정 시 기본값; exp는 --results_dir의 마지막 경로 요소). 예시 경로:
...
전 그림은 다크 테마(#0f1117 배경 · 흰 텍스트 · 14pt+ · dpi 170)로 통일된다. 산출물:
  ...
  - 타겟별 → results/{exp}/presentation/{target}/ :
      confusion_matrix · pr_roc_curve · threshold_sweep ·
...
reliability/calibration · t-SNE 는 포팅하지 않는다(전자는 report_artifacts가 생성, 후자는 입력 부재).
...
from src.data.dataset import CLASS_NAME_LIST, CUT, DANGER  # noqa: E402
...
def _compute_ece(df: pd.DataFrame, n_bins: int = _ECE_BINS) -> float:
    """저장된 prob로 max-softmax 신뢰도 기반 ECE 계산 (uncalibrated, 빠른 표시용)."""
    probs, yt = _probs(df), _labels(df, "true_label")
    conf = probs.max(1)
    correct = (probs.argmax(1) == yt).astype(float)
...
def _danger_risk_coverage(df: pd.DataFrame) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """danger one-vs-rest threshold sweep → (coverage, precision, miss_rate)."""
    score = _probs(df)[:, DANGER_IDX]
    y = (_labels(df, "true_label") == DANGER_IDX).astype(int)
    total_pos = int(y.sum())
    cov, prec, miss = [], [], []
    for t in np.linspace(0.0, 1.0, 101):
        ...
        if n_pred == 0:
            continue
        tp = int((pred & (y == 1)).sum())
        cov.append(n_pred / len(y))
        prec.append(tp / n_pred)
        miss.append(1.0 - tp / total_pos if total_pos else np.nan)
    order = np.argsort(cov)
    return np.array(cov)[order], np.array(prec)[order], np.array(miss)[order]
...
def plot_risk_coverage(results_dir: Path, targets: list[str], out: Path) -> Path:
    ...
    for target in targets:
        ...
        cov, prec, miss = _danger_risk_coverage(df)
...
def plot_kpi_board(summary_csv: Path, results_dir: Path, out: Path,
    ...
    ece = _compute_ece(pd.read_csv(preds)) if preds.exists() else None
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Hazard-detection 발표용 추가 시각화 (추론 재실행 없음)")
    ...
    for target in targets:
        ...
        print(f"[OK] {target} 추가 플롯 완료 → {out}/")
